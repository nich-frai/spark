...
const customBoundary = "-----------------------------168072sdfs82475249we41622650073";
...
parser.write(buf, (e) => {
    ...
    process.stdout.write(`Took ${performance.now() - start} ms to process ${buf.length} bytes`);
    process.exit(0);
});
function createMultipartBody(options) {
    const body = Buffer.concat([
        ...
        ...Array.from({ length: options.numberOfParts }).map((_v, i) => {
            const partHeader = Buffer.from(`content-disposition: form-data; name="a";filename="a.test"\r\ncontent-type: image/jpg\r\n\r\n`);
